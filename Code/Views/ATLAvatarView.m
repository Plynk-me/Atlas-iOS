...
- (void)lyr_commonInit
{
    ...
    // Do not initialise the presence indicator at all; it is not hiding correctly on the list of conversations using the existing mechanisms.
    // Presence Status View
//    _presenceStatusView = [[ATLPresenceStatusView alloc] init];
//    _presenceStatusEnabled = false;
...
{
    // Always hide presence indicator for now as we cannot turn them off everywhere that we want to
//    self.presenceStatusView.hidden = !presenceStatusEnabled;
//    _presenceStatusEnabled = presenceStatusEnabled;
    self.presenceStatusView.hidden = true;
    _presenceStatusEnabled = false;
}
...
- (void)layoutSubviews
{
    ...
    // Overrides default code to create circular avatar images and instead chooses between circles and squares
    self.imageView.layer.cornerRadius = (self.radiusOverride == 0) ? avatarViewDiameter / 2 : self.radiusOverride;
    [self invalidateIntrinsicContentSize];
